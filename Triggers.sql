...
select @nowcatid=BCatId from Book where BId=@bid
...
declare @catidcount int
declare @nowcatidcount int

select @catidcount=count(BId) from Book where BCatId=@catid and BStatus=1
select @nowcatidcount=count(BId) from Book where BCatId=@nowcatid and BStatus=1

update Category set CatCount = @catidcount where CatId=@catid
update Category set CatCount = @nowcatidcount where CatId=@nowcatid
...
select @numpos=count(@nowcatid) from Book where BCatId=@nowcatid
select @catsum=sum(BPosition) from Book where BCatId=@nowcatid
select @cpos=@catsum/@numpos
update Category set CatPosition=@cpos where CatId=@nowcatid
